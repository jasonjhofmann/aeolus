...
- **0.4 — Household IAQ priority (this deployment; see [[household-iaq-health-profile]]).** The reference home is an **allergy/MCAS household** (occupant with Mast Cell Activation Syndrome + eczema; both occupants severe allergies) with a **leaky envelope** (sliding glass doors). Consequences that override the generic design:
  ...
  - **Depressurizing (exhaust-dominant) strategies are health-penalized, not just energy-penalized:** with leaky doors, any net depressurization pulls **unfiltered** outdoor allergens straight through the envelope. So **prefer the balanced, filtered ERV**; down-weight exhaust/induced strategies (FR-L2 cost) for this household even though there is *no* combustion-backdraft risk (see §7.5).
  - **Cooperate with the air purifiers, don't fight them:** purifiers remove PM but not CO₂; Aeolus removes CO₂ but must not undo their work by dragging in unfiltered air.
...
- **FR-G3 Outdoor-AQ veto (co-primary objective — see §0.4).** Outdoor air imports PM2.5/PM10/ozone/smoke/pollen. **Block outdoor-air strategies (windows, high ERV intake, supply) when the configured outdoor-AQ sensor(s) exceed threshold.** Surface the CO₂-vs-PM trade-off explicitly; never silently swap hazards. For allergy/MCAS households the threshold is **low and multi-pollutant** (PM2.5 *and* pollen), and an outdoor-air strategy that would raise indoor PM is rejected even if CO₂ is high — CO₂ is never worth importing allergens. Note: a *balanced ERV with a filtered intake* is preferred precisely because it adds outdoor air without the unfiltered-infiltration penalty of depressurization.
