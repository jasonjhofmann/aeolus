...
async def test_diagnostics_dump_has_debug_fields(hass: HomeAssistant) -> None:
    ...
    assert {"min_on_sec", "min_off_sec", "override_window_sec", "spaces", "actuators"} <= diag.keys()
    space = diag["spaces"][0]
    ...
    assert {"outdoor_air_vetoed", "max_runtime_exceeded", "is_overridden"} <= actuator.keys()
